                ...
                ->arrayNode('tracing')
                    ...
                    ->children()
                        ...
                        ->arrayNode('doctrine')
                            ...
                            ->end()
                        ->end()
                        ->arrayNode('assetstorage')
                            ->addDefaultsIfNotSet()
                            ->children()
                                ->scalarNode('memory_directory')
                                    ->defaultValue('/dev/shm/assetstorage')
                                    ->info('Select a directory inside an in-memory filesystem')
                                    ->example('/dev/shm/some-directory')
                                ->end()
                                ->enumNode('adapter')
                                    ->defaultNull()
                                    ->values([null, 'null', 'local', 'google'])
                                    ->info('One of: null, local, google')
                                    ->example('local')
                                ->end()
                                ->arrayNode('local_adapter')
                                    ->children()
                                        ->scalarNode('target_dir')
                                            ->info('Directory where to save stored assets')
                                            ->example('/path/to/var/assetstorage')
                                        ->end()
                                    ->end()
                                ->end()
                                ->arrayNode('google_adapter')
                                    ->children()
                                        ->scalarNode('key_file')
                                            ->info('Path to google authentication json key file')
                                            ->example('/path/to/google_auth_key.json')
                                        ->end()
                                        ->scalarNode('bucket')
                                            ->info('Name of Google Cloud Storage bucket')
                                            ->example('otel-traces-store')
                                        ->end()
                                    ->end()
                                ->end()
                            ->end()
